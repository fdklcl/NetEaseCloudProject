...
void PictureWidget::setPictureScreen() {
    ...
    //添加对应图片
    for (int i = 1; i <= RAW_PICTURE_MAX_NUMBER; i++) {
        m_PixmapList.append(QPixmap(QString(":/images/picturewall/%1.png").arg(i)));
    }

    for (int i = 0; i < RAW_PICTURE_MAX_NUMBER; i++) {
    ...
    //放入映射map中 key:buttonId value:pictureItem
    for(int i = 0; i < RAW_PICTURE_MAX_NUMBER; i++) {
...
void PictureWidget::timerOutFunc() {
    QVector<QGraphicsItemAnimation*> item(RAW_PICTURE_MAX_NUMBER);
    for (int i = 0; i < RAW_PICTURE_MAX_NUMBER; i++) {
...
void PictureWidget::on_btnL_clicked() {
    ...
    qDebug() << "luckyfan test on_btnL_clicked id = " << id;
    rollItem(id,0);
    m_newT->start();
}
